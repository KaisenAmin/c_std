...
**Return Value**: A pointer to the newly created `Tuple`, or `NULL` if memory allocation fails.

**Usage Case**: Used to create a new tuple with a specified number of elements.
...
**Usage Case**: Used to clean up and free memory when a tuple is no longer needed.
...
**Return Value**: `true` if the data is successfully set, `false` otherwise.

**Usage Case**: Used to modify a tuple by inserting or replacing data at a specified position.
...
**Return Value**: A pointer to the data at the specified index, or `NULL` if an error occurs.

**Usage Case**: Used to access data stored at a specific index in the tuple, useful for retrieving dynamically allocated content.
...
**Return Value**: A pointer to the newly created `Tuple`, or `NULL` if creation or element setting fails.

**Usage Case**: Used to create a tuple and initialize it with multiple pieces of data in one function call.
...
**Return Value**: A pointer to the newly created `Tuple`, or `NULL` if the creation fails.

**Usage Case**: Useful for creating a tuple that references existing variables without copying their values.
...
**Usage Case**: Used when two tuples need to swap their elements and sizes.
...
**Return Value**: A pointer to the newly created `Tuple`, or `NULL` if creation fails.

**Usage Case**: Useful for passing a set of arguments as a tuple without copying the data.
...
**Return Value**: The number of elements in the tuple, or 0 if the tuple is `NULL`.

**Usage Case**: Used to retrieve the number of elements in a tuple.
...
**Return Value**: `true` if the tuples are equal, `false` otherwise.

**Usage Case**: Used to check if two tuples are identical in size and content.
...
**Return Value**: `true` if `t1` is less than `t2`, `false` otherwise.

**Usage Case**: Used to compare two tuples lexicographically based on their size and content.
...
**Return Value**: `true` if `t1` is greater than `t2`, `false` otherwise.

**Usage Case**: Used to compare two tuples lexicographically and determine if one is greater than the other.
...
**Return Value**: `true` if `t1` and `t2` are not equal, `false` otherwise.

**Usage Case**: Used to check if two tuples are different in size or content.
...
**Return Value**: `true` if `t1` is greater than or equal to `t2`, `false` otherwise.

**Usage Case**: Used to determine if the first tuple is greater than or equal to the second one.
...
**Return Value**: `true` if `t1` is less than or equal to `t2`, `false` otherwise.

**Usage Case**: Used to determine if the first tuple is less than or equal to the second one.
...
**Return Value**: `true` if the tuple is empty or if `t` is `NULL`, `false` otherwise.

**Usage Case**: Used to verify if a tuple is empty or has no elements.
...
int main() {
    ...
    IntArray3 arrayValue;
    int arrayData[] = {1, 2, 3};
    for (int i = 0; i < 3; i++) {
        array_at(arrayValue, i) = arrayData[i];
    }
    IntArray3 *arrayPtr = &arrayValue;

    int queueData = 10;
    queue_push(queueValue, &queueData);

    tuple_set(myTuple, 0, &stringValue, sizeof(String *));
    tuple_set(myTuple, 1, &arrayPtr,    sizeof(IntArray3 *));
    tuple_set(myTuple, 2, &queueValue,  sizeof(Queue *));

    size_t size;

    String** retrievedString = (String**)tuple_get(myTuple, 0, &size);
    IntArray3** retrievedArray = (IntArray3**)tuple_get(myTuple, 1, &size);
    ...
    if (retrievedArray) {
        int element = array_at(**retrievedArray, 1);
        fmt_printf("Array element[1]: %d\n", element);
    }
    if (retrievedQueue) {
...
## Example 10: forward tuple to functions as parameter with `tuple_forward_as_tuple`
...
int main() {
    ...
    /* Pass `name` (the String*) and `&age` (an int*) — both are
       single-indirection pointers from the tuple's point of view. */
