...
static MemoryPoolVector *memory_pool_create(size_t size) {
    ...
    if (!pool) {
        fprintf(stderr, "Error: Failed to allocate memory for MemoryPoolVector structure.\n");
        return NULL;
    }

    pool->pool = malloc(size);
    if (!pool->pool) {
        fprintf(stderr, "Error: Failed to allocate memory for memory pool of size %zu.\n", size);
        free(pool);
        return NULL;
    }
...
static void *memory_pool_allocate(MemoryPoolVector *pool, size_t size) {
    if (!pool) {
        fprintf(stderr, "Error: Memory pool is not initialized.\n");
        return NULL;
    }
    if (size == 0) {
        fprintf(stderr, "Error: Cannot allocate zero size.\n");
        return NULL;
    }
    if (pool->used + size > pool->poolSize) {
        fprintf(stderr, "Error: Memory pool out of space. Cannot allocate %zu bytes.\n", size);
...
static void memory_pool_destroy(MemoryPoolVector *pool) {
    if (!pool) {
        fprintf(stderr, "Error: Attempted to destroy a non-initialized memory pool.\n");
        return;
    }
...
Vector* vector_create(size_t itemSize) {
    ...
    if (!vec){
        fprintf(stderr, "Error: Can not allocate memory for Vector structure");
        exit(-1);
    } 
    ...
    if (!vec->pool) {
        free(vec);
        fprintf(stderr, "Can not allocate memory for Vector pool");
        exit(-1);
    }
    ...
    if (!vec->items) {
        ...
        free(vec);
        fprintf(stderr, "Error: Can not allocate memory for Vector items");
        exit(-1);
    }
    return vec;
}
...
bool vector_is_equal(const Vector* vec1, const Vector* vec2) {
    if (vec1 == NULL || vec2 == NULL) {
        fprintf(stderr, "Error: One or both vector pointers are NULL in vector_is_equal.\n");
        return false; // One or both vectors are NULL, so they cannot be equal
    }
    if (vec1->size != vec2->size) {
        fprintf(stderr, "Error: Vectors have different sizes in vector_is_equal.\n");
...
bool vector_is_less(const Vector* vec1, const Vector* vec2) {
    if (!vec1 || !vec2) {
        fprintf(stderr, "Error: One or both vector pointers are NULL in vector_is_less.\n");
        return false;
    }
...
bool vector_is_greater(const Vector* vec1, const Vector* vec2) {
    if (!vec1 || !vec2) {
        fprintf(stderr, "Error: One or both vector pointers are NULL in vector_is_greater.\n");
        return false;
    }
...
bool vector_is_not_equal(const Vector* vec1, const Vector* vec2) {
    if (!vec1 || !vec2) {
        fprintf(stderr, "Error: One or both vector pointers are NULL in vector_is_not_equal.\n");
        return true;
    }
    if (vec1->size != vec2->size) { 
        fprintf(stderr, "Error: Vectors have different sizes in vector_is_not_equal.\n");
        return true;
    }
    return memcmp(vec1->items, vec2->items, vec1->size * vec1->itemSize) != 0;
}
...
bool vector_is_greater_or_equal(const Vector* vec1, const Vector* vec2) {
    if (!vec1 || !vec2) {
        fprintf(stderr, "Error: One or both vector pointers are NULL in vector_is_greater_or_equal.\n");
...
bool vector_is_less_or_equal(const Vector* vec1, const Vector* vec2) {
    if (!vec1 || !vec2) {
        fprintf(stderr, "Error: One or both vector pointers are NULL in vector_is_less_or_equal.\n");
        return false; // Handle the error as per your application's needs
    }
...
void vector_erase(Vector *vec, size_t pos, size_t len) {
    if (vec == NULL) {
        fprintf(stderr, "Error: Vector is NULL in vector_erase.\n");
        return; // Handle the error as per your application's needs
    }

    if (pos >= vec->size) {
        fprintf(stderr, "Error: Position is out of bounds in vector_erase.\n");
        return; // Handle the error as per your application's needs
    }

    if (pos + len > vec->size) {
        fprintf(stderr, "Error: Length is too large in vector_erase.\n");
        return; // Handle the error as per your application's needs
    }
...
void vector_insert(Vector *vec, size_t pos, void *item) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_insert.\n");
        return; // Handle the error as per your application's needs
    }
    if (pos > vec->size) {
        fprintf(stderr, "Error: Position is out of bounds in vector_insert.\n");
        return; // Handle the error as per your application's needs
    }
    ...
    if (vec->size == vec->capacitySize) {
        ...
        if (!newItems) {
            fprintf(stderr, "Error: Failed to allocate memory for vector_insert.\n");
...
void vector_reserve(Vector *vec, size_t size) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_reserve.\n");
        return; // Handle the error as per your application's needs
    }
    ...
    if (!newItems) { 
        fprintf(stderr, "Error: Failed to allocate memory for vector_reserve.\n");
...
void vector_resize(Vector *vec, size_t size) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_resize.\n");
        return; // Handle the error as per your application's needs
    }
...
void vector_shrink_to_fit(Vector *vec) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_shrink_to_fit.\n");
        return; // Handle the error as per your application's needs
    }
    if (vec->size == vec->capacitySize) {
    ...
    if (!newItems) { 
        fprintf(stderr, "Error: Failed to allocate memory for vector_shrink_to_fit.\n");
        return; // Handle allocation failure, maybe by resizing the pool or another appropriate action
    }
...
void vector_swap(Vector *vec1, Vector *vec2) {
    if (!vec1 || !vec2) {
        fprintf(stderr, "Error: One or both vectors are NULL in vector_swap.\n");
        return; // Handle the error as per your application's needs
    }
...
void vector_assign(Vector *vec, size_t pos, void *item) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_assign.\n");
        return; // Handle the error as per your application's needs
    }
    if (pos >= vec->size) {
        fprintf(stderr, "Error: Position is out of bounds in vector_assign.\n");
        return; // Handle the error as per your application's needs
    }
    memcpy((char *)vec->items + pos * vec->itemSize, item, vec->itemSize);
}
...
void vector_emplace(Vector *vec, size_t pos, void *item, size_t itemSize) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_emplace.\n");
        return; // Handle the error as per your application's needs
    }
    if (pos > vec->size || itemSize != vec->itemSize) {
        fprintf(stderr, "Error: Invalid position or item size in vector_emplace.\n");
        return; // Handle the error as per your application's needs
    }
    if (vec->size == vec->capacitySize) {
...
void vector_emplace_back(Vector *vec, void *item, size_t itemSize) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_emplace_back.\n");
        return; // Handle the error as per your application's needs
    }
    if (itemSize != vec->itemSize) {
        fprintf(stderr, "Error: Invalid item size in vector_emplace_back.\n");
        return; // Handle the error as per your application's needs
    }
    if (vec->size >= vec->capacitySize) {
        vector_reserve(vec, vec->capacitySize * 2); // Use the modified version
...
void vector_push_back(Vector *vec, const void *item) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_push_back.\n");
        return; // Handle the error as per your application's needs
    }
    if (vec->size >= vec->capacitySize) {
        ...
        void *newItems = memory_pool_allocate(vec->pool, newCapacity * vec->itemSize);
        if (!newItems) {
            fprintf(stderr, "Error: Failed to allocate memory in vector_push_back.\n");
            return; // Handle allocation failure, maybe by resizing the pool or another appropriate action
        }
...
void vector_deallocate(Vector *vec) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_deallocate.\n");
        return; // Handle the error as per your application's needs
    }
    if (vec->pool != NULL) {
        memory_pool_destroy(vec->pool);
...
void *vector_at(const Vector *vec, size_t pos) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_at.\n");
    ...
    if (pos < vec->size) {
        ...
        fprintf(stderr, "Error: Position is out of bounds in vector_at.\n");
        return NULL; // Handle the error as per your application's needs
    }
}
...
void *vector_rbegin(Vector *vec) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_rbegin.\n");
        return NULL; // Handle the error as per your application's needs
    }
    if (vec->size == 0) {
        fprintf(stderr, "Error: Vector is empty in vector_rbegin.\n");
        return NULL; // Handle the error as per your application's needs
    }
...
void *vector_rend(Vector *vec) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_rend.\n");
        return NULL; // Handle the error as per your application's needs
    }
...
const void *vector_cbegin(Vector *vec) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_cbegin.\n");
        return NULL; // Handle the error as per your application's needs
    }
    if (vec->size == 0) {
        fprintf(stderr, "Error: Vector is empty in vector_cbegin.\n");
        return NULL; // Handle the error as per your application's needs
    }
...
const void *vector_cend(Vector *vec) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_cend.\n");
        return NULL; // Handle the error as per your application's needs
    }
    if (vec->size == 0) {
        fprintf(stderr, "Error: Vector is empty in vector_end.\n");
        return NULL; // Handle the error as per your application's needs
    }
...
const void *vector_crbegin(Vector *vec) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_crbegin.\n");
        return NULL; // Handle the error as per your application's needs
    }
    if (vec->size == 0) {
        fprintf(stderr, "Error: Vector is empty in vector_crbegin.\n");
        return NULL; // Handle the error as per your application's needs
    }
...
void *vector_begin(Vector *vec) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_crend.\n");
        return NULL; // Handle the error as per your application's needs
    }
...
void *vector_end(Vector *vec) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_end.\n");
        return NULL; // Handle the error as per your application's needs
    }
    if (vec->size == 0) {
        fprintf(stderr, "Error: Vector is empty in vector_end.\n");
...
void *vector_pop_back(Vector *vec) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_pop_back.\n");
        return NULL; // Handle the error as per your application's needs
    }
    if (vec->size == 0) {
        fprintf(stderr, "Error: Vector is empty in vector_pop_back.\n");
        return NULL; // Handle the error as per your application's needs
    }
...
void vector_clear(Vector *vec) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_clear.\n");
        return; // Handle the error as per your application's needs
    }
    ...
    if (vec->capacitySize > reducedCapacity) {
        ...
        if (newItems != NULL || reducedCapacity == 0) {
            ...
            vec->capacitySize = reducedCapacity;
             ...
             fprintf(stderr, "Error: Cannot reallocate the Vector in vector_clear.\n");
        }
    }
}
...
void *vector_front(Vector *vec) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_front.\n");
        return NULL; // Handle the error as per your application's needs
    }
    if (vec->size == 0) {
        fprintf(stderr, "Error: Vector is empty in vector_front.\n");
        return NULL; // Handle the error as per your application's needs
    }
...
void *vector_back(Vector *vec) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_back.\n");
        return NULL; // Handle the error as per your application's needs
    }
    if (vec->size == 0) {
        fprintf(stderr, "Error: Vector is empty in vector_back.\n");
        return NULL; // Handle the error as per your application's needs
    }
...
void *vector_data(Vector *vec) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_data.\n");
        return NULL;
    }
...
size_t vector_size(const Vector *vec) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_size.\n");
...
size_t vector_capacity(Vector *vec) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_capacity.\n");
        return 0; // Handle the error as per your application's needs
    }
...
size_t vector_max_size(Vector *vec) {
    if (!vec) {
        fprintf(stderr, "Error: Vector is NULL in vector_max_size.\n");
        return 0; // Handle the error as per your application's needs
    }
    return vec->itemSize;
}
