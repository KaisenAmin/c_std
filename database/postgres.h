 ...
 * live above their DEFINITIONS in postgres.c (file-level convention).
 *
 * Thin object-shaped wrapper over libpq.
 */
...
/* #define POSTGRES_LOGGING_ENABLE */
    ...
    #define POSTGRES_LOG(fmt, ...) \
        do { fprintf(stderr, "[POSTGRES LOG] " fmt "\n", ##__VA_ARGS__); } while (0)
#else
    #define POSTGRES_LOG(...) do { } while (0)
#endif
...
bool             postgres_create_function              (Postgres* pg, const char* functionName, const char* returnType, const char* language,
                                                        const char* functionBody, const char* paramDefinitions);
...
bool             postgres_create_trigger               (Postgres* pg, const char* triggerName, const char* tableName,
                                                        const char* timing, const char* event, const char* function);
...
/* ------------------------------------------------------------------ */
/* Connection-attribute accessors                                     */
/* ------------------------------------------------------------------ */
