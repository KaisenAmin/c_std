 ...
 * live above their DEFINITIONS in sort.c (file-level convention).
 */
...
/* Generic element type — sort routines operate on `ElementType[]`,
 * where each slot holds a user pointer (e.g. `int*`). */
...
/* Algorithm selector for `sort_generic`. */
typedef enum {
    SORT_INSERTION,
    SORT_QUICKSORT,
    SORT_MERGESORT,
...
/* Per-run statistics filled in by `sort_generic` (and inspectable
 * via `sort_print_stats`). */
typedef struct {
    size_t comparisons;
    size_t swaps;
...
/* ------------------------------------------------------------------ */
/* Parallel variants (basic implementations)                          */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/* Generic dispatcher (algorithm selectable at call time)             */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/* Default print callbacks for common types                           */
/* ------------------------------------------------------------------ */
